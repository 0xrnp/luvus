...
skills), bohay ships a **module-authoring skill**. It teaches the agent this whole
guide: the manifest tables, the `BOHAY_*` environment, the callback methods, and
...
Prefer to read or copy it by hand? [View the skill on GitHub](https://github.com/RizRiyz/bohay/blob/main/skills/bohay-module/SKILL.md) (use the raw view's copy button).
...
`contexts` adds items to **bohay's** menus. To give **your own dock rows** a
...
```sh
"$BOHAY_BIN_PATH" ui dock push --id you:boards --title BOARDS --rows '[
  {
...
offers three actions while the `build` row below it offers none. Bohay can't
guess a menu for a row it didn't create, so you declare it. A row with no `menu`
has no context menu, exactly as before.

The action receives the same `BOHAY_MODULE_ROW_*` variables a left-click passes
(with `BOHAY_MODULE_ROW_VALUE` set to the entry's `value` when it has one), plus
`BOHAY_MODULE_ACTION_ID` telling it which item was picked, so one script can
serve the whole menu:

```sh
case "$BOHAY_MODULE_ACTION_ID" in
  flash)   idf.py -p "$BOHAY_MODULE_ROW_VALUE" flash ;;
  erase)   idf.py -p "$BOHAY_MODULE_ROW_VALUE" erase-flash ;;
esac
```
...
`destructive` is only a colour: bohay won't ask "are you sure" for you. If an
