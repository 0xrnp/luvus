---
title: CLI Commands
description: Every bohay command, generated from `bohay help all` and drift-checked in CI.
---

:::note[Source of truth]
The binary is authoritative: **`bohay help all`** always matches your installed
version. This page carries the same text **verbatim**. A test in bohay's CI
fails if they ever drift apart.
:::

Use `bohay help <topic> [command]` for focused help. Most commands also accept
a trailing `--help`. Pass-through commands preserve that flag as payload, so
use the explicit `bohay help` form for them. Help writes to stdout, while
