...
{ "text": string,
  ...
  "action": string,
  "value":  string,
  "menu":   [ {"title": string, "action": string, "destructive"?: bool} ] }
```
...
absent `menu` means the row has no context menu. Menu entries also set
`BOHAY_MODULE_ACTION_ID`, so one script can handle the whole menu.
