...
| tabs | `tab.list` · `tab.new` · `tab.focus` · `tab.move` · `tab.rename` · `tab.close` |
...
`$BOHAY_PANE_ID` when available; a raw API request with no pane uses the
...
The equivalent CLI commands are `bohay workspace rename <i> <name>`,
`bohay workspace pin <i>`, and `bohay workspace unpin <i>`. Renaming changes
...
`tab.move` reorders the active workspace's tab vector. `to` is the desired final
position, and the currently active tab remains active even when its number
changes. Zero, missing, same-position, and out-of-range values return
...
`target` is a live alias, numeric pane ID, or a unique agent kind. Bohay uses
...
declares the dock in its manifest and pushes its content here, and bohay owns the
rendering.
...
`BOHAY_MODULE_ROW_VALUE` so one action can serve every row.
...
also set `BOHAY_MODULE_ACTION_ID`, so one script can handle the whole menu.

The CLI wrappers are `bohay ui dock push|list|move` (`push` also reads its rows
from stdin).
