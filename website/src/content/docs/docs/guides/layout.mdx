...
```sh
bohay pane move 7 --tab 2       # existing tab in pane 7's workspace
bohay pane move 7 --new-tab     # append a fresh tab for pane 7
bohay tab move 3 1              # move current workspace tab 3 to position 1
```

All tab numbers are 1-based. A pane move keeps its process running and follows
it with focus. Tab reordering preserves whichever tab was active.
...
A file path opens **in bohay**, not in some other app. It goes through exactly the
